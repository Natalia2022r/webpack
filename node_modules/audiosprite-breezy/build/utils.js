...
exports.getPromisesChain = exports.getChosenAudioOptions = exports.getExportFormatsOptions = void 0;
function getAllExportFormatsOptions(options) {
    const formats = {
        aiff: [],
        wav: [],
...
function getExportFormatsOptions(options) {
    const formats = getAllExportFormatsOptions(options);
    if (!options.export.length)
        return formats;
    const output = {};
    options.export.forEach((key) => { output[key] = formats[key]; });
    return output;
}
exports.getExportFormatsOptions = getExportFormatsOptions;
function getChosenAudioOptions(options, format) {
    const formats = getAllExportFormatsOptions(options);
    return formats[format];
}
exports.getChosenAudioOptions = getChosenAudioOptions;
function getPromisesChain(...promiseFunctions) {
    return promiseFunctions.reduce((promiseChain, currentPromiseFunction) => (promiseChain.then(currentPromiseFunction)), Promise.resolve());
}
exports.getPromisesChain = getPromisesChain;
